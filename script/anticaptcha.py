...
class Anticaptcha(object):
    ...
    def __wait_for_task_result(self, task_id):
        ...
        while (time.time() - start_time) < self.timeout_time_sec:
            response = self.__post_request(
                self.get_task_result_url, json=content)
            if response.get('status') == 'ready':
                return response
            time.sleep(1)
        raise AnticaptchaException(
            'Timeout {} reached '.format(self.timeout_time_sec))
    ...
    def solve_recaptcha(self, website_url, website_key):
        ...
        solution = self.__wait_for_task_result(
            task_id)['solution']['gRecaptchaResponse']
